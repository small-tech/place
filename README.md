...
__For Linux, macOS, and Windows 10.__

The [Small Web](https://ar.al/2020/08/07/what-is-the-small-web/) is for people (not startups, enterprises, or governments).
...
## Feature Highlights
  ...
  - __Just works.__ No configuration; [get started in seconds](https://place.small-web.org/#get-started).
  ...
  - __Secure by default.__
    ...
    Your server will score [an A+](https://www.ssllabs.com/ssltest/analyze.html?d=place.small-web.org) on the [SSL Labs SSL Server Test](https://www.ssllabs.com/ssltest).
  ...
  - __Supports [DotJS](#dotjs) for dynamic routes.__ (DotJS is PHP-like simple routing for Node.js introduced by Place for quickly prototyping and building dynamic sites).

  - For sites that implement the [Small Web](https://ar.al/2020/08/07/what-is-the-small-web/) conventions, download and deploy your site using the simple [pull and push commands](#pull-and-push).
...
__(Note: all commands should be run in your regular account, not as root.)__ Place will refuse to run if launched from the root account.

### Native binaries

__Before you pipe any script into your computer, always view the source code ([Linux and macOS](https://should-i-pipe.it/https://place.small-web.org/install), [Windows](https://should-i-pipe.it/https://place.small-web.org/install.txt)) and make sure you understand what it does.__

#### Linux

```shell
wget -qO- https://place.small-web.org/install | bash
```
...
```shell
curl -s https://place.small-web.org/install | bash
```

#### Windows 10 with PowerShell running under Windows Terminal

```shell
iex(iwr -UseBasicParsing https://place.small-web.org/install.txt).Content
```

### Node.js

```shell
npm i -g @small-tech/place
```
...
```shell
wget -qO- https://place.small-web.org/install | bash -s -- beta
```

__Note:__ On Macs, `wget` is not installed by default but `curl` is so you can use that instead:

```shell
curl -s https://place.small-web.org/install | bash -s -- beta
```
...
Any recent Linux distribution should work. However, Place is most thoroughly tested at Small Technology Foundation on Ubuntu 20.04/Pop!_OS 20.04 (development and staging) and Ubuntu 18.04 LTS (production).
...
macOS 10.14.x Mojave and macOS 10.15.x Catalina are supported.
...
__Windows Subsystem for Linux (WSL) is _not_ supported.__ (You can install and run Place under WSL but seamless TLS certificate handling for local servers will not work out of the box as WSL and Windows 10 do not share certificate stores. If you do want to use Place under WSL, you have to first install Place on Windows 10 and run a local server (`place`) to create the certificate authority and certificates, then install and run Place under WSL and then manually copy the contents of `~/.small-tech.org/place/tls/local/` from Windows 10 to WSL.)

_Production use is not possible under Windows._

## Dependencies

Place tries to seamlessly install the dependencies it needs when run. That said, there are certain basic components it expects on a Linux-like system. These are:

  - `sudo`
  - `bash` (on Linux, macOS, etc.) or `PowerShell` running under [Windows Terminal](https://github.com/Microsoft/Terminal) (on Windows 10).
  - `wget` or `curl` (on Linux and macOS) are required to download the installation script when installing Place using the one-line installation command. On Linux, you can install either via your distribution’s package manager (e.g., `sudo apt install wget` on Ubuntu-like systems). macOS comes with `curl` installed.

If it turns out that any of these prerequisites are a widespread cause of first-run woe, we can look into having them installed automatically in the future. Please [open an issue](https://github.com/small-tech/Place/issues) if any of these affects you during your deployments or in everyday use.

### Automatically-installed dependencies

__For production use, passwordless sudo is required.__ On systems where the sudo configuration directory is set to `/etc/sudoers.d`, Place will automatically install this rule. On other systems, you might have to [set it up yourself](https://serverfault.com/questions/160581/how-to-setup-passwordless-sudo-on-linux).

__For localhost servers__, the bundled [mkcert](https://github.com/FiloSottile/mkcert) requires [certutil](https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSS/tools/NSS_Tools_certutil) and the [Network Security Services](https://developer.mozilla.org/en-US/docs/Mozilla/Projects/NSS) (NSS) dynamic libraries. Place will attempt to automatically install the required libraries using popular package managers. <strike>Please note that this will fail on PinePhones running UBPorts as NSS is missing from the apt package manager for that distribution.</strike> ([The PinePhone issue](https://bugzilla.mozilla.org/show_bug.cgi?id=1652739) has been resolved.)
...
This command will automatically restart a running Place daemon if one exists. If you are running Place as a regular process, it will continue to run and you will run the newer version the next time you launch a regular Place process.

__Note:__ There is a bug in the semantic version comparison in the original release with the update feature (version 12.9.5) that will prevent upgrades between minor versions (i.e., between 12.9.5 and 12.10.x and beyond). This was fixed in version 12.9.6. If you’re still on 12.9.5 and you’re reading this after we’ve moved to 12.10.0 and beyond, please stop Place if it’s running and [install the latest Place](#install) manually.
...
This is a primary security feature given that Place is meant for use by individuals, not startups or enterprises with operations teams that can (in theory, at least) maintain servers with the latest updates.
...
Note that if your current working directory is inside a special subfolder of your site (`.dynamic`, `.wildcard`, `.db`), Place magically does the right thing and serves the site root instead of the folder you’re in. If you really do want to serve one of these folders or a subfolder thereof, specifically state your intent by passing the current folder (`.`) as an argument.
...
__Note:__ Place will refuse to serve the root directory or your home directory for security reasons.
...
Use a service like [PageKite](https://pagekite.net/) (or ngrok Pro or above) to point a custom domain name to your temporary staging server. Make sure you set your `hostname` file (e.g., in `/etc/hostname` or via `hostnamectl set-hostname <hostname>` or the equivalent for your platform) to match your domain name. The first time you hit your server via your hostname it will take a little longer to load as your Let’s Encrypt certificates are being automatically provisioned by Auto Encrypt.
...
You can use the `pull` and `push` commands if your local and remote setup adheres to the following [Small Web](https://ar.al/2020/08/07/what-is-the-small-web/) conventions:
...
#### Remote
  ...
  - __Account name:__ `small-web`
  - __Folder being served:__ `/home/small-web/public`
...
__Please make sure that you are NOT running as root.__ Place will refuse to run if launched from the root account.
...
For example, if you run the command on a connected server that has the ar.al domain pointing to it and `ar.al` set in _/etc/hostname_, you will be able to access the site at https://ar.al. (Yes, of course, [ar.al](https://ar.al) runs on Place.) The first time you hit your live site, it will take a little longer to load as your Let’s Encrypt certificates are being automatically provisioned by Auto Encrypt.
...
Place uses the [systemd](https://freedesktop.org/wiki/Software/systemd/) to start and manage the daemon. Beyond the commands listed above that Place supports natively (and proxies to systemd), you can make use of all systemd functionality via the [systemctl](https://www.freedesktop.org/software/systemd/man/systemctl.html) and [journalctl](https://www.freedesktop.org/software/systemd/man/journalctl.html) commands.

## Build and test from source

Place is built using and supports Node.js 14 LTS (currently version 14.15.0).

The build is created using Nexe and our own pre-built Nexe base Node.js binaries hosted on place.small-web.org. Please make sure that the version of your Node.js runtime matches the currently supported version stated above to ensure that the correct Nexe binary build is downloaded and used by the build script.
...
git clone https://github.com/small-tech/place.git app
cd app
./install
...
__Note:__ for commands that require root privileges (i.e., `enable` and `disable`), Place will automatically restart itself using sudo and Node must be available for the root account. If you’re using [nvm](https://github.com/creationix/nvm), you can enable this via:

```shell
# Replace v14.15.0 with the version of node you want to make available globally.
sudo ln -s "$NVM_DIR/versions/node/v14.15.0/bin/node" "/usr/local/bin/node"
sudo ln -s "$NVM_DIR/versions/node/v14.15.0/bin/npm" "/usr/local/bin/npm"
```

If you forget to do this and run `place enable`, you will find the following error in the systemctl logs: `/etc/systemd/system/place.service:15: Executable "node" not found in path`. The command itself will fail with:

```
Error: Command failed: sudo systemctl start place
Failed to start place.service: Unit place.service has a bad unit file setting.
See system logs and 'systemctl status place.service' for details.
```
...
dist/linux/<binary-version>/place test/site
```
...
### Deploying Place itself

(You will most likely not need to do this.)

```shell
# To cross-compile binaries for Linux (x64), macOS, and Windows
# and also copy them over to the Place web site for deployment.
npm run deploy
```
...
### Commands:
  ...
  - `serve`: Serve specified folder on specified `@host` (at `:port`, if given). The order of arguments is:
...
#### For both the `serve` and `enable` commands:
  ...
  - `--aliases`: Comma-separated list of additional domains to obtain TLS certificates for and respond to. These domains point to the main domain via a 302 redirect. Note that the _www_ alias is not added automatically. To specify it, you can use the shorthand form:`--aliases=www`
...
All command-line arguments are optional. By default, Place will serve your current working folder over port 443 with locally-trusted certificates.
...
| Serve folder demo on port 666             | place serve demo @localhost:666                                |

### Pull from and push to your remote server

| Goal                                      | Command                                                        |
| ----------------------------------------- | -------------------------------------------------------------- |
| Pull (download) from remote server        | place pull                                                     |
| Push (deploy) to remote server            | place push                                                     |
...
|                                           | place serve demo @hostname                                     |

#### Start-up daemon:

| Goal                                      | Command                                                        |
| ----------------------------------------- | -------------------------------------------------------------- |
| Install and serve current folder as daemon| place enable                                                   |
| Ditto & also ensure it can rsync via ssh  | place enable --ensure-can-sync                                 |
| Get status of daemon                      | place status                                                   |
...
e.g., The example from [the test site](https://github.com/small-tech/Place/blob/master/test/site/404/index.html) uses the following code:
...
When creating your own servers (see [API](#API)), you can generate the default error pages programmatically using the static methods `Place.default404ErrorPage()` and `Place.default500ErrorPage()`, passing in the missing path and the error message as the argument, respectively to get the HTML string of the error page returned.

## Ephemeral statistics

When Place launches, you will see a line similar to the following in the console:

```
📊    ❨Place❩ For statistics, see https://localhost/b64bd821d521b6a65a307c2b83060766
```
...
__Note:__ You can remind yourself of the statistics URL while running the Place daemon in production using the `place status` command while the server is active.
...
To test it, run a local server (`place`) and go to `https://localhost`. Refresh the page a couple of times to see the counter increase.
...
Place will load your dynamic route at startup and you can test it by hitting _https://localhost/server-stats_ using a local web server. Each time you refresh, you should get the latest dynamic content.
...
Now run a local server (`place`) and hit `https://localhost/hello/Laura` to see `Hello, Laura!` in the browser.
...
Now run a local server (`place`) and hit:
...
Now if you run `place` on the root of your web folder (the one that contains the _.dynamic_ folder) and hit `https://localhost/cows`, you should get a random cow in a random colour every time you refresh.
...
When you save this update, Place will automatically reload the server with your new code. When you refresh in your browser, you should see exactly the same behaviour as before.
...
Place is not limited to HTTPS, it also supports secure WebSockets.
...
To test it out, run Place and then open up the JavaScript console in a couple of browser windows and enter the following code into them:
...
Your Place server has a global database called `db` that you can use from any route.
...
The _routes.js_ file should export a function that accepts a reference to the Express app created by Place and defines its routes on it. For example:
...
So, if Place finds a _routes.js_ file in the root folder of your site’s folder, it will only use the routes from that file (it will not apply file-based routing).

If Place cannot find a _routes.js_ file, it will look to see if separate _.https_ and _.wss_ folders have been defined (the existence of just one of these is enough) and attempt to load DotJS routes from those folders. (If it finds separate _.get_ or _.post_ folders within the _.https_ folder, it will add the relevant routes from those folders; if it can’t it will load GET-only routes from the _.https_ folder and its subfolders.)

If separate _.https_ and _.wss_ folders do not exist, Place will expect all defined DotJS routes to be HTTPS and will initially look for separate _.get_ and _.post_ folders (the existence of either is enough to trigger this mode). If they exist, it will add the relevant routes from those folders and their subfolders.

Finally, if Place cannot find separate _.get_ and _.post_ folders either, it will assume that any DotJS routes it finds in the _.dynamic_ folder are HTTPS GET routes and attempt to add them from there (and any subfolders).

### Directory paths in your application

Your dynamic web routes are running within Place, which is a Node application compiled into a native binary. Here are how the various common directories for Node.js apps will behave:

  - `os.homedir()`: __(writable)__ This is the home folder of the account running Place. You can write to it to store persistent objects (e.g., save data). __Note:__ since Place can restart itself using sudo when it needs elevated privileges, do not use this. Instead, use `Util.unprivilegedHomeDirectory()` to always get a reference to the home directory of the unprivileged account (instead of the home directory for `root`). On Windows, this method will simply return the value of `os.homedir()`.
  ...
  - `/`: __(read-only)__ Path to the `/usr` folder (Place is installed at `/usr/local/place`). You should not have any reason to use this.

If you want to access the directory of Place itself (e.g., to load in the `package.json` to read the app’s version), you can use the following code:

```js
const appPath = require.main.filename.replace('bin/place', '')
```
...
If all you want to do is to customise the behaviour of your pages using client-side JavaScript based on parameters provided through the URL path, you don’t have to use dynamic routes and a `routes.js` file, you can use wildcard routes instead, which are much simpler.
...
When Place finds a `.wildcard` folder, it adds every first-level sub-folder in it as a route that maps to the `index.html` file in it. In the example above, `/hello/aral` and `hello/what/is/this/about` will both map to the same file.

Similarly, any HTML file found in the `.wildcard` folder will also be mapped to a wildcard route. So you could have created the same route in `.wildcard/hello.html` instead.

Any path fragment after the route name itself is treated as a positional argument.

Although you could parse the `document.location` yourself to get at the arguments and the route name, Place makes it even easier for you by injecting a tiny bit of JavaScript at the top of your page that exposes these as:
...
You can also include Place as a Node module into your Node project. This section details the API you can use if you do that.

Place’s `createServer` method behaves like the built-in _https_ module’s `createServer` function. Anywhere you use `require('https').createServer`, you can simply replace it with:

```js
const Place = require('@small-tech/place')
new Place().createServer
```

### createServer([options], [requestListener])

  - __options__ _(object)_: see [https.createServer](https://nodejs.org/api/https.html#https_https_createserver_options_requestlistener). Populates the `cert` and `key` properties from the automatically-created [Auto Encrypt Localhost](https://source.small-tech.org/Place/lib/auto-encrypt-localhost) or Let’s Encrypt certificates and will overwrite them if they exist in the options object you pass in. If your options has `options.global = true` set, globally-trusted TLS certificates are obtained from Let’s Encrypt using [Auto Encrypt](https://source.small-tech.org/Place/lib/auto-encrypt).

  - __requestListener__ _(function)_: see [https.createServer](https://nodejs.org/api/https.html#https_https_createserver_options_requestlistener). If you don’t pass a request listener, Place will use its default one.
...
const server = new Place().createServer(options, app).listen(443, () => {
...
Options is an optional parameter object that may contain the following properties, all optional:
  ...
  - __global__ _(boolean)_: if true, globally-trusted Let’s Encrypt certificates will be provisioned (if necessary) and used via Auto Encrypt. If false (default), locally-trusted certificates will be provisioned (if necessary) and used using _Auto Encrypt Localhost_.
    ...
    __Returns:__ Place instance.

__Note:__ if you want to run the site on a port < 1024 on Linux, ensure that privileged ports are disabled ([see details](https://source.small-tech.org/Place/app/-/issues/169)). e.g., use:
...
### serve(callback)
  ...
  - __callback__ _(function)_: a function to be called when the server is ready. This parameter is optional. A default callback is provided.
...
```js
const Place = require('@small-tech/place')
const server = new Place({global: true}).serve()
```

## Troubleshooting

This section documents exotic issues that you might run into that are not bugs in Place and details how to can fix them.
