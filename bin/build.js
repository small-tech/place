...
///////////////////////////////////////////////////////////////////////////
//
// Builds Linux x86/ARM/ARM64, macOS, and Windows 10 binaries of Place.
//
// Run with: npm run build
//       or: npm run deploy
//
// The Place binary is built from a Nexe base image. If the images
// do not exist locally, they are automatically downloaded from the
// remote we host at place.
//
...
if (commandLineOptions.deploy) {
  // Ensure that a working copy of the Place web site exists
  // locally where we ex[ect it to so we can deploy to it.
  if (!fs.existsSync(websitePath)) {
    console.log('❌ Error: No local working copy of Place web site found.\n')
    console.log(`   Please clone https://small-tech.org/place/site to ${websitePath}`)
...
console.log(`\n ⚙️ Place build started on ${presentBinaryVersion(binaryVersion)}.\n
...
const remote           = 'https://place.small-web.org/nexe/'
const build            = true
...
// Resources
//
// These are assets and code that are necessary for Place to work but which
...
const resources = [
  ...
  // The cliui library uses an exports map which Nexe doesn’t currently know how to handle
  // so we need to include these modules manually as they’re not discovered from the .cjs automatically.
  'node_modules/string-width/',
  'node_modules/strip-ansi/',
  'node_modules/wrap-ansi/'
]

const input = 'bin/place'
...
async function buildBinary () {
  ...
  if (commandLineOptions.install) {
    ...
    try {
      childProcess.execSync('QUIET=true place disable')
    } catch (error) {
      // Ignore error. It just means Place was not enabled; which is what we want.
    }
    ...
    if (isWindows) {
      const windowsInstallationDirectory =  'C:\\Program Files\\place'
  ...
  // Only zip and copy files to the local working copy of the Place web site if explicitly asked to.
  // Check that a local working copy of the Place web site exists at the relative location
  // that we expect it to. If it doesn’t skip this step.
  if (commandLineOptions.deploy) {

    // Update the install file and deploy them to the Place web site.
    console.log('   • Updating the installation scripts and copying them to local Place web site working copy.')
    ...
    //
    // Copy Place release binaries to the Place web site.
    //
    // Note: this requires a relative directory setup that matches the project structure
    // ===== of the Place source code repository. Remember we are running in:
    // place/app/bin/
    //
    // place
    //  |_ app                    This project.
    //  |   |_ bin                The folder that this script is running in.
    //  |_ site                   The Place web site.
    //      |_ version/
    ...
    //
    // If it cannot find the Place web site, the build script will simply skip this step.
    //

    console.log('   • Copying release binaries to the Place web site…')
    ...
    // Write out a dynamic route on the place.small-web.org web site to return the binary version.
    // This endpoint is used by the auto-update feature to decide whether the binary should be updated.
    console.log(`   • Adding dynamic binary version endpoint for ${releaseChannel} version to Place web site.`)
